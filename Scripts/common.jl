...
function speed_test(rng::RandomNumbers.AbstractRNG{T}, n) where {T<:Number}
    A = Array{T}(undef, n)
    rand!(rng, A)
...
function speed_test(rng::MersenneTwister, n)
    T = UInt64
    A = Array{T}(undef, n)
    rand!(rng, A)
    elapsed = @elapsed rand!(rng, A)
...
function test_all(rng::Random.AbstractRNG, n)
